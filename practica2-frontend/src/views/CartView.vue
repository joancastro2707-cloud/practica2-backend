<template>
  <div style="font-family: sans-serif; padding: 30px; max-width: 900px; margin: 0 auto;">
    ...
    <div v-if="carrito.items.length === 0" style="padding: 30px; background: #f8f9fa; text-align: center; border-radius: 8px; border: 1px dashed #ccc;">
      <p style="color: #666; font-size: 18px; margin-bottom: 20px;">Tu carrito está actualmente vacío.</p>
    ...
    <div v-else>
      <div style="background: white; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
        <table style="width: 100%; border-collapse: collapse; text-align: left;">
          <thead style="background: #f1f1f1; color: #444; font-weight: bold;">
            <tr>
          ...
          <tbody>
            <tr v-for="item in carrito.items" :key="item.id" style="border-bottom: 1px solid #eee;">
              ...
              <td style="padding: 15px; text-align: center;">
                <button @click="carrito.eliminarDelCarrito(item.id)" style="background: #dc3545; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;">×</button>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div style="padding: 20px; background: #f8f9fa; display: flex; justify-content: space-between; align-items: center; border-top: 2px solid #ddd;">
          ...
          <div style="text-align: right;">
            <span style="font-size: 14px; color: #666; text-transform: uppercase; font-weight: bold; display: block;">Total General:</span>
            <h2 style="margin: 0; color: #28a745; font-size: 32px; font-weight: bold;">${{ carrito.totalPrecio.toFixed(2) }}</h2>
          </div>
        </div>
      </div>

      <button @click="finalizarCompra" style="width: 100%; background: #28a745; color: white; border: none; padding: 15px; font-size: 18px; font-weight: bold; border-radius: 8px; margin-top: 20px; cursor: pointer; box-shadow: 0 4px 10px rgba(40,167,69,0.3);">
        🚀 Finalizar la Compra (Enviar Orden)
      </button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const confirmarVaciar = () => {
      if (confirm('¿Estás completamente seguro de que deseas remover todos los artículos de tu carrito?')) {
        carrito.vaciar()
      }
    }

    const finalizarCompra = () => {
      alert('🎉 ¡Pedido procesado con éxito! Se ha mandado el estado de tu carrito al backend.')
      carrito.vaciar()
    }

    return { carrito, confirmarVaciar, finalizarCompra }
  }
}
</script>